...
const FilesSpace = () => {
  ...
  const [preview, setPreview] = useState({ open: false, name: "", content: "" });
  ...
  const fetchFiles = async (type) => {
    ...
    try {
      ...
      return Array.isArray(data) ? data.map(f => ({ name: f.FileName, content: f.Content })) : [];
    } catch (err) {
  ...
  const openPreview = (name, content) => {
    setPreview({ open: true, name, content });
  };

  const closePreview = () => {
    setPreview({ open: false, name: "", content: "" });
  };
              ...
              <ul className="files-list">
                {sharedStorageFiles.map((file, idx) => (
                  <li
                    key={`shared-${idx}`}
                    className="file-item"
                    onClick={() => openPreview(file.name, file.content)}
                  >
